...
lambda_0 = c / freq     #波長λ
impedance = 50
degree = np.arange(0, 360, 1)   #位相差用角度
amplitude = 1           #振幅
Tx_Power = 1
Gain_Tx = 0             #ゲイン
...
distances = np.sqrt(x_diff**2 + y_diff**2) / 100   #7x30x30  100はcmからmへの変換
phases_0 = (distances % lambda_0) / lambda_0 * 2 * np.pi    #基準送信源

Gf = -10 * np.log10((4*np.pi*distances/lambda_0)**2) + (Gain_Tx + Gain_Rx)  # 自由空間のゲイン
Rx_P = Tx_Power + Gf
dbuV = Rx_P + 10 * np.log10(impedance) + 90     #dbm_to_v関数の中身
Rx_Amp = (10 ** (dbuV/20)) / 1e6

complex_signal_0 = np.sqrt(Rx_Amp) * np.exp(1j * phases_0)  #基準送信源の複素信号
comp_0 = complex_signal_0.reshape(7, 900)   #7x900に変形            
result = np.zeros([7,900,900], dtype=np.complex)    #計算結果を入れる箱
...
for i in tqdm(range(int(0), int(360), 1)):
    phase_offset = (i / 360 * np.pi * 2)
    phases = phases_0 + phase_offset        #基準プラス位相差
    
    #この時点で距離と位相が用意できているので，次は電力の計算(位相差ありの方)
    complex_signal = np.sqrt(Rx_Amp) * np.exp(1j * phases)

    #全パターンの足し合わせ
    comp = complex_signal.reshape(7, 900)
    for i in range(900):
        result[:,:,i] = comp_0 + comp[:,i].reshape(7,1)
